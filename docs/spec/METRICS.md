...
- **Definition:** Median and P90 days from a GitHub PR's `created_at` to the earliest **non-self** review event on it, among PRs with at least one qualifying review, bucketed by the PR's creation month. The GitHub-PR-specific analog of `review_latency` above — the same underlying "time to first review" concept, computed from GitHub's own review events instead of the git-trailer/JIRA-reviewer-field proxy, since GitHub PRs cover only part of Cassandra's review activity (DECISIONS.md "Cassandra-specific facts"). **Self-reviews excluded (orchestrator review, issue #54 fixup):** GitHub records a PR author's own replies inside review threads as `COMMENTED` review events by that same author — these are not a *reviewer's* review-latency signal and must never count as "the first review." Compared at the identity level: `resolved_identity` (issue #52's `github_login` identity_link rows) when a link exists, falling back to the raw login string otherwise; an unresolvable side (e.g. a deleted account) is never treated as a self-match.
...
- **Formula:** Median/P90 of `t_first_review − t_pr_opened` over PRs with ≥1 non-self review, bucketed by PR creation month.
- **Population & exclusions:** §0.5, §0.6 (floor 5). Bot-authored PRs/reviews already excluded at collection time (`collectors/github.py`'s own bot-login filtering). The PR author's own review events are excluded (see Definition).
- **Window:** monthly.
...
- **Definition:** Two related numbers reported together (never collapsed into one, per `reviewer_top_k_share`'s own convention): mean unique **non-self** reviewers per PR, and mean/median non-self reviews per PR, among GitHub PRs that received ≥1 qualifying review in a completed calendar month (bucketed by review submission month). **Self-reviews excluded (orchestrator review, issue #54 fixup):** same exclusion and identity-level compare as `pr_time_to_first_review` above — a PR author's own review events never count toward unique-reviewers-per-PR, reviews-per-PR, or the total unique-reviewer count; a PR whose only review activity is a self-review does not enter the population at all.
...
- **Formula:** For PRs with ≥1 non-self review in month `m`: `mean(unique_reviewers_per_pr)` (the metric's `value`) and `mean(reviews_per_pr)`/`median(reviews_per_pr)` (`details_json`), all counting only non-self reviews.
- **Population & exclusions:** §0.5, §0.6 (floor 5 PRs). Counts raw GitHub logins directly, not `resolved_identity`, for cross-source reviewer identity — identity resolution (#6) doesn't yet cover `github_login` raw identifiers for that purpose (`metrics/dev_metrics.py` module docstring); `resolved_identity` *is* consulted specifically for self-review exclusion (see Definition), which is a narrower, single-purpose use.
- **Window:** monthly.
