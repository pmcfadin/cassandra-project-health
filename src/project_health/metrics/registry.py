...
_DESCRIPTIONS: dict[str, str] = {
    ...
    "pr_time_to_first_review": (
        "Median and P90 days from pr.created_at to the earliest pr_review.submitted_at for "
        "that PR, among PRs with at least one review, bucketed by the PR's creation month "
        "(the 'ready for review' moment); dense monthly. details_json carries p90_days and n. "
        "GitHub-PR-specific analog of METRICS.md §3's review_latency (which is computed from "
        "the git-trailer + JIRA-reviewer-field proxy instead) -- reported as its own metric "
        "rather than merged into review_latency, since the two draw from different, only "
        "partially-overlapping evidence (DECISIONS.md 'Cassandra-specific facts': GitHub PRs "
        "cover only part of review activity). Tier: proxy. Dimension: reviewer capacity. "
        "Role: supporting (review_latency already holds this dimension's latency key slot). "
    ...
    "pr_review_engagement": (
        "Review engagement across GitHub PRs that received at least one review in a "
        "completed calendar month (bucketed by pr_review.submitted_at): value = mean unique "
        "reviewers per PR; details_json carries mean_reviews_per_pr, median_reviews_per_pr, "
        "n_prs (the population n), n_reviews and n_unique_reviewers_total (per-source "
        "counts, D2.3 audit trail). Two related but distinct numbers reported together, "
